...
const excuses: [string, string, number, number][] = [
  ["s4d0", "works on my laptop", 182, 110],
  ["s4d1", "no tracing", 992, 120],
  ["s4d2", "streamlit demo, one user", 79, 610],
  ["s4d3", "prompt injection", 1050, 640],
  ["s4d4", "who pays for tokens?", 614, 80],
];
...
export default function MLOps() {
  return (
    <>
      {excuses.map(([name, text, left, top]) => (
        <div key={name} className="card anim" style={{ animationName: name, left, top }}>{text}</div>
      ))}
